          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="flex items-center gap-2 bg-slate-900/50 p-2 rounded border border-slate-700">
                ...
                <span className="text-sm">攻击: {Math.floor(leader.baseAttack * Math.pow(leaderLevel, 1.2))}</span>
              </div>
              <div className="flex items-center gap-2 bg-slate-900/50 p-2 rounded border border-slate-700">
                <Heart className="w-4 h-4 text-green-400" />
                <span className="text-sm">生命: {Math.floor(leader.baseHp * Math.pow(leaderLevel, 1.2))}</span>
              </div>
            </div>
          </div>
        </div>
                ...
                <div className="flex flex-col gap-2 mt-2">
                  <div className="flex justify-between items-center">
                    ...
                    {!isEquipped && (
                      <button 
                        onClick={() => {
                          if (!currentGenerals[0]) equipGeneral(0, g.id);
                          else if (!currentGenerals[1]) equipGeneral(1, g.id);
                          else alert('上阵位置已满');
                        }}
